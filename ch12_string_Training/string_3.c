#include <stdio.h> //문자열 문제 3번 - 문자열 개수 세기
...
int main(void)
{
    ...
    printf("문자열을 입력하시오: ");//gets함수: 문자열을 입력받는 함수
    gets(arr_str);
    
    printf("개수를 셀 문자를 입력하시오: ");//개수를 셀 문자 입력받기
    scanf("%c", &count_c);

    printf("%c의 개수: %d\n", count_c, str_chr(arr_str, count_c));//결과-문자열 개수 count해주는 "함수 호출"
    return 0;
}
